...
export default function OrdersList() {
  ...
  useEffect(() => {
    ...
    async function fetchOrders() {
      try {
        const data = await getOrders(user.user_id || user.id)
  ...
  const handleDownloadInvoice = (order) => {
    ...
    const paymentMethod = 'Tarjeta / PSE'
          ...
          <div class="invoice-card">
            ...
            <div class="header-layout">
              ...
              <div class="invoice-meta">
                ...
                <p><strong>Fecha Emisión:</strong> ${new Date(order.created_at || Date.now()).toLocaleDateString('es-CO')}</p>
                <p><strong>Fecha Vencimiento:</strong> ${new Date(order.created_at || Date.now()).toLocaleDateString('es-CO')}</p>
              </div>
            </div>
  ...
  if (loading) {
    return (
      <main className="orders-page empty-state">
  ...
  return (
    <main className="orders-page" style={{ padding: '2rem', maxWidth: '1000px', margin: '0 auto', minHeight: 'calc(100vh - 80px)' }}>
      <h1 style={{ marginBottom: '2rem', fontSize: '2rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
      ...
      {orders.length === 0 ? (
        <div style={{
          ...
          <h3 style={{ fontSize: '1.5rem', marginBottom: '0.5rem' }}>Aún no tienes pedidos</h3>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {orders.map(order => (
            <div key={order.id} style={{ 
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-color, #f0f0f0)', paddingBottom: '1rem' }}>
                <div>
                  <h3 style={{ margin: 0 }}>Pedido #{order.id}</h3>
                  <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>
                    {new Date(order.created_at || Date.now()).toLocaleDateString('es-CO', { year: 'numeric', month: 'long', day: 'numeric' })}
                  </span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', 
                  color: order.status === 'CONFIRMED' || order.status === 'FULFILLED' ? 'var(--primary-color, #2F6F52)' : '#f59e0b',
                  fontWeight: 600, fontSize: '0.9rem',
                  background: order.status === 'CONFIRMED' || order.status === 'FULFILLED' ? 'rgba(47, 111, 82, 0.1)' : 'rgba(245, 158, 11, 0.1)',
                  padding: '0.5rem 1rem', borderRadius: '20px'
                }}>
                  {order.status === 'CONFIRMED' || order.status === 'FULFILLED' ? <CheckCircle size={16} /> : <Clock size={16} />}
                  <span>{order.status === 'PENDING' ? 'Pendiente' : order.status === 'CONFIRMED' ? 'Confirmado' : order.status}</span>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', marginTop: '0.5rem' }}>
                ...
                {order.items.map(item => (
                  <div key={item.book_id} style={{
                    ...
                    <span style={{ fontWeight: 600, color: 'var(--text-primary, #333)' }}>{item.quantity}</span>
                    <span style={{ color: 'var(--text-primary, #333)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', paddingRight: '1rem' }}>
                      {item.book_title}
                    </span>
                    <span style={{ fontWeight: 600, textAlign: 'right', color: 'var(--text-primary, #333)' }}>
              ...
              <div style={{ 
                ...
                <div style={{ fontWeight: 700, fontSize: '1.15rem', color: 'var(--text-primary)' }}>
                  Total Paid: <span style={{ color: '#fbbf24' }}>${order.total_amount.toLocaleString('es-CO')}</span>
                </div>
              </div>
            </div>
          ))}
