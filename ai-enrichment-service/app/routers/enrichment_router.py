...
def _push_enrichment_to_catalog(book_id_str: str, result: dict) -> None:
    ...
    if update_payload:
        try:
            resp = requests.put(
                f"{CATALOG_SERVICE_URL}/books/{book_id}",
                json=update_payload,
                timeout=5,
            )
            if not resp.ok:
                logger.warning("Catalog PUT /books/%d returned %d: %s", book_id, resp.status_code, resp.text[:200])
        except requests.RequestException as exc:
            logger.warning("Failed to update catalog book %d: %s", book_id, exc)

    try:
        resp = requests.patch(
            f"{CATALOG_SERVICE_URL}/books/{book_id}/mark-enriched",
            timeout=5,
        )
        if not resp.ok:
