...
INVENTORY_BASE = "http://inventory-service:8002"
TIMEOUT = 3.0


def _batch_to_summary(b: dict) -> BatchSummary:
    total = b.get("total_rows", 0)
    errors = b.get("error_rows", 0)
    ...
    return BatchSummary(
        ...
        status=b.get("status", ""),
        total_rows=total,
        valid_rows=b.get("valid_rows", 0),
        error_rows=errors,
        error_rate=rate,
    )


def _use_mock_batches() -> List[dict]:
    return MOCK_BATCHES


def _use_mock_errors(batch_id: int) -> List[dict]:
    return MOCK_ERRORS.get(batch_id, [])
...
async def get_batch_report(batch_id: int) -> Optional[BatchQualityReport]:
    try:
        ...
        if batch_raw is None:
            return None
        errors_raw = _use_mock_errors(batch_id)

    total = batch_raw.get("total_rows", 0)
    error_count = batch_raw.get("error_rows", 0)
    rate = round(error_count / total, 4) if total else 0.0
    ...
    return BatchQualityReport(
        batch_id=batch_id,
        filename=batch_raw.get("filename", ""),
